    ...
    <div className="animate-fade-in space-y-6">
          ...
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setCategory(cat)}
              className={`px-3 py-1.5 text-xs font-medium rounded-full transition-all ${category === cat
                ...
                : 'bg-surface-muted dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:text-ink dark:hover:text-white'}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-72 bg-white dark:bg-gray-900 rounded-2xl animate-pulse border border-gray-100 dark:border-gray-800" />
          ))}
        </div>
      ) : error ? (
            ...
            return (
              <div key={course.id} className="card-hover group flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <h3 className="text-sm font-semibold text-ink dark:text-white leading-snug mb-2 line-clamp-2 group-hover:text-brand-600 transition-colors">
                    {course.title}
                  </h3>
                  ...
                  <div className="mt-auto flex gap-2">
                    <button onClick={() => navigate(`/courses/${course.id}`)}
                      className="flex-1 py-2 text-xs font-medium text-brand-600 bg-brand-50 dark:bg-brand-900/20 rounded-xl hover:bg-brand-100 dark:hover:bg-brand-900/30 transition-colors">
                      View Details
                    </button>
                    <Button
                      size="sm"
