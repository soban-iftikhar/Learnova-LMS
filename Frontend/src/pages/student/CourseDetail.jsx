...
import {
  enrollmentsApi, quizzesApi, assignmentsApi,
  videosApi, ratingsApi,
} from '../../api/index'
...
export default function CourseDetailPage() {
  ...
  // Load course info
  const {
    data: course, loading: courseLoading, error: courseError, refetch: refetchCourse,
  } = useAsync(() => coursesApi.getById(id), [id])

  // Load content — only after course is loaded
  ...
  // Load all student enrollments to check if already enrolled
  const {
    data: myEnrollData, refetch: refetchEnrollments,
  } = useAsync(() => enrollmentsApi.getMyCourses({ size: 200 }), [])
  ...
  // Find this course's enrollment record
  const myEnrollment = (myEnrollData?.content || [])
    .find(e => String(e.course?.id) === String(id))
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('already')) {
        // Already enrolled — just refresh the enrollment list
        refetchEnrollments()
  ...
  return (
    <div className="animate-fade-in">
      <Link to="/courses"
        className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-ink mb-6 transition-colors group">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-gray-100 text-sm text-gray-400">
                  {course.instructor?.name && (
                    <span className="flex items-center gap-1.5">
                      <BookOpen size={14} />{course.instructor.name}
                    </span>
                  )}
                  ...
                  <span className="flex items-center gap-1.5">
                    <Play size={14} />{videos.length} lectures
                  </span>
                  <span className="flex items-center gap-1.5">
                    <ClipboardList size={14} />{quizzes.length} quizzes
                  </span>
                  <span className="flex items-center gap-1.5">
                    <FileText size={14} />{assignments.length} assignments
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="card overflow-hidden">
            <div className="p-5 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {videos.length + quizzes.length + assignments.length} items
              </p>
            </div>

            {/* Lectures */}
            <SectionAccordion
              label="Video Lectures" icon={Play}
              count={videos.length}
              open={openSection === 'lectures'}
              ...
              {videos.map((v, i) => (
                <button key={v.id}
                  ...
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold ${activeVideo?.id === v.id ? 'bg-brand-500 text-white' : 'bg-brand-50 text-brand-600'}`}>
                    {i + 1}
                  </div>
                  ...
                  {!isEnrolled
                    ...
                    : <Play size={14} className="text-brand-500 flex-shrink-0" />
                  }
                </button>
              ))}
              {!videos.length && (
                <p className="px-5 py-4 text-sm text-gray-400">No lectures added yet.</p>
              )}
            </SectionAccordion>
            ...
            <SectionAccordion
              ...
              {!quizzes.length && (
                <p className="px-5 py-4 text-sm text-gray-400">No quizzes available yet.</p>
              )}
            </SectionAccordion>
            ...
            <SectionAccordion
              ...
              {assignments.map(a => {
                ...
                return (
                  <div key={a.id} className="flex items-center gap-3 px-5 py-3 border-t border-gray-50">
                    ...
                    {isEnrolled
                      ? <Button size="xs" variant="outline" onClick={() => setSubmitModal(a)}>
              ...
              {!assignments.length && (
                <p className="px-5 py-4 text-sm text-gray-400">No assignments yet.</p>
              )}
            </SectionAccordion>
          </div>
        </div>
            ...
            {isEnrolled ? (
              <>
                ...
                <Button fullWidth onClick={() => setRatingModal(true)}>
                  <CheckCircle2 size={15} /> Rate this course
                </Button>
              ...
              <>
                ...
                <p className="text-sm text-gray-400">
                  Enroll to access all lectures, quizzes and assignments.
                </p>
      ...
      {/* Modals */}
      {quizModal && (
        <QuizAttemptModal quiz={quizModal} onClose={() => setQuizModal(null)} />
      )}
      {submitModal && (
        <AssignmentSubmitModal assignment={submitModal} onClose={() => setSubmitModal(null)} />
      )}
      {ratingModal && (
        <RatingModal
          courseId={id}
      ...
      {/* Chat bubble — only for enrolled students */}
      {isEnrolled && (
        <CourseChat
          courseId={id}
...
/* ─── Section Accordion ─────────────────────────────────────────────────────── */
function SectionAccordion({ label, icon: Icon, count, open, onToggle, children }) {
  return (
    <div>
    ...
    try {
      const res = await quizzesApi.submit(quiz.id, {
        ...
        answers: Object.entries(answers).map(([qId, ans]) => ({
          question_id: Number(qId), answer: ans,
        })),
      })
      setResult(res.data)
      setPhase('result')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm"
        onClick={phase === 'start' ? onClose : undefined} />
              ...
              {questions.map((q, i) => (
                <div key={q.id} className="space-y-3">
                  <div className="flex items-start gap-3">
                    <span className="w-7 h-7 rounded-full bg-brand-100 text-brand-700 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                      {i + 1}
                    </span>
                        ...
                        <button key={oi}
                          ...
                          className={`w-full text-left px-4 py-3 rounded-xl text-sm border-2 transition-all ${
                            selected
                              ...
                              : 'border-gray-100 hover:border-brand-300 text-ink'
                          }`}>
          ...
          {phase === 'result' && result && (
            <div className="text-center space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="card p-4 text-center">
                  <p className="text-3xl font-bold text-ink">{Math.round(result.percentage)}%</p>
...
function AssignmentSubmitModal({ assignment, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!file && !text.trim()) {
      toast.warning('Please attach a file or add notes.')
      return
    }
    setLoading(true)
    try {
    ...
    <Modal isOpen onClose={onClose} title={`Submit: ${assignment.title}`} size="md">
      {submitted ? (
        <div className="text-center py-8">
          ...
          <h3 className="font-semibold text-ink">Submitted!</h3>
          <p className="text-sm text-gray-400 mt-1">Your assignment has been submitted successfully.</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-sm font-medium text-ink-muted block mb-2">
              Attach File (PDF, DOC, ZIP)
            </label>
            <div
              ...
              <input id="asgn-file" type="file" className="hidden"
    ...
    <Modal isOpen onClose={onClose} title="Rate Your Experience" size="sm">
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400">
            How would you rate {instructorName || 'the instructor'}?
          </p>
        </div>
        <div className="flex justify-center gap-2">
          {[1, 2, 3, 4, 5].map(n => (
            <button key={n} type="button"
              ...
              <Star size={32}
                className={`transition-colors ${n <= (hover || rating) ? 'text-amber-400 fill-amber-400' : 'text-gray-200'}`} />
            </button>
          ))}
        </div>
        {rating > 0 && (
